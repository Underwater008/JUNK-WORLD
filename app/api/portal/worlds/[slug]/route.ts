...
import { deleteWorld, updateWorldDraft } from "@/lib/worlds/server";
...
function toErrorResponse(error: unknown) {
  ...
  return NextResponse.json({ error: "World update failed." }, { status: 500 });
}
...
export async function PUT(
  ...
  try {
    ...
    const result = await updateWorldDraft(slug, body);
    return NextResponse.json({
...
export async function DELETE(
  ...
  try {
    const result = await deleteWorld(slug);
